...
import { useDnD } from "../../hooks/useDragAndDrop/useDragAndDrop";
...
function SelectionArea(props: Props) {
  ...
  const addElement = (newObj: TextBlock | ImageBlock | GraphicBlock) => {
    switch (newObj.type) {
      case "text":
        return <Text text={newObj} />; //* поменять переданные элементы на stateMenu. Может возникнуть проблема с типами.
      case "graphic":
        return (
          <Graphic
            form={newObj.form}
